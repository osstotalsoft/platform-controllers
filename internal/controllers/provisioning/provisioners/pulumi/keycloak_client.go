...
func deployKeycloakClient(target provisioning.ProvisioningTarget,
	...
	if spec.ServiceAccountsEnabled && len(spec.ServiceAccountRealmRoles) > 0 {
		...
		for _, role := range spec.ServiceAccountRealmRoles {
			_, err = openid.NewClientServiceAccountRealmRole(ctx, fmt.Sprintf("%s-%s-service-account-role", keycloakClient.Name, role),
				&openid.ClientServiceAccountRealmRoleArgs{
					RealmId:              pulumi.String(realm.Id),
	...
	if spec.Organization != "" && spec.ServiceAccountsEnabled {
		err = addToOrganization(target, client, ctx, keycloakClient.Name, realm.Id, spec.Realm, spec.Organization)
		if err != nil {
			return nil, err
		}
	}
...
func addToOrganization(target provisioning.ProvisioningTarget,
	client *openid.Client, ctx *pulumi.Context, keycloakClientName string,
	realmId string, realm string, organization string) error {

	// Get the service account user associated with this client
	saUser := openid.GetClientServiceAccountUserOutput(ctx, openid.GetClientServiceAccountUserOutputArgs{
		RealmId:  pulumi.String(realmId),
		ClientId: client.ID().ToStringOutput(),
	}, nil)
	...
	if keycloakURL == "" || keycloakClientID == "" || keycloakClientSecret == "" {
		return fmt.Errorf("KEYCLOAK_URL, KEYCLOAK_CLIENT_ID and KEYCLOAK_CLIENT_SECRET environment variables must be set")
	}
	tc := provisioning.GetTemplateContext(target)
	orgId, err := template.ParseTemplate(organization, tc)
	if err != nil {
		return err
	}
	...
	_, err = local.NewCommand(ctx,
		fmt.Sprintf("%s-add-member-organization", keycloakClientName),
		&local.CommandArgs{
			Create: createCmd,
