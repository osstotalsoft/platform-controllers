...
| `SKIP_INITIAL_RECONCILE` | `skipInitialReconcile` | Set to `true` to discard the reconcile work items generated by the informers' initial list on startup, so a controller restart doesn't run `pulumi up`/refresh against every existing stack. Resources created or changed while the controller was down are only picked up once something else touches the same tenant/domain again |
