...
const (
	...
	// EnvSkipInitialReconcile, when set to "true", drops the reconcile work items generated by the
	// informers' initial list on startup, so a controller restart does not trigger a pulumi up/refresh
	// for every existing stack. Genuine changes that happen after startup are still processed normally.
	EnvSkipInitialReconcile = "SKIP_INITIAL_RECONCILE"
)
...
type ProvisioningController struct {
	...
	// handlerRegistrations is used to tell when every event handler has been notified of its initial
	// list. Unlike the informer caches' own sync state, this accounts for the asynchronous delivery of
	// events to the handlers themselves.
	handlerRegistrations []cache.ResourceEventHandlerRegistration
...
func NewProvisioningController(clientSet clientset.Interface,
	...
	skipInitialReconcile, _ := strconv.ParseBool(os.Getenv(EnvSkipInitialReconcile))
	...
	c.registerHandler(addTenantHandlers(c.tenantInformer, c.enqueueTenant))
	c.registerHandler(addPlatformHandlers(c.platformInformer))
	c.registerHandler(addTenantCategoryHandlers(c.tenantCategoryInformer, c.enqueueTenantsByCategory))

	c.registerHandler(addResourceHandlers[*provisioningv1.HelmRelease]("Helm release", c.helmReleaseInformer.Informer(), c.enqueueDomain))
	c.registerHandler(addResourceHandlers[*provisioningv1.HelmReleaseV2]("Helm release v2", c.helmReleaseV2Informer.Informer(), c.enqueueDomain))
	c.registerHandler(addResourceHandlers[*provisioningv1.MsSqlDatabase]("MsSql database", c.mssqlDbInformer.Informer(), c.enqueueDomain))
	c.registerHandler(addResourceHandlers[*provisioningv1.LocalScript]("Local script", c.localScriptInformer.Informer(), c.enqueueDomain))
	c.registerHandler(addResourceHandlers[*provisioningv1.MinioBucket]("Minio bucket", c.minioBucketInformer.Informer(), c.enqueueDomain))
	c.registerHandler(addResourceHandlers[*provisioningv1.KeycloakClient]("Keycloak client", c.keycloakClientInformer.Informer(), c.enqueueDomain))

	if azureEnabled {
		c.registerHandler(addResourceHandlers[*provisioningv1.AzureDatabase]("Azure database", c.azureDbInformer.Informer(), c.enqueueDomain))
		c.registerHandler(addResourceHandlers[*provisioningv1.AzureManagedDatabase]("Azure managed database", c.azureManagedDbInformer.Informer(), c.enqueueDomain))
		c.registerHandler(addResourceHandlers[*provisioningv1.AzurePowerShellScript]("Azure PowerShell script", c.azurePowerShellScriptInformer.Informer(), c.enqueueDomain))
		c.registerHandler(addResourceHandlers[*provisioningv1.AzureVirtualMachine]("Azure virtual machine", c.azureVirtualMachineInformer.Informer(), c.enqueueDomain))
		c.registerHandler(addResourceHandlers[*provisioningv1.AzureVirtualDesktop]("Azure virtual Desktop", c.azureVirtualDesktopInformer.Informer(), c.enqueueDomain))
		c.registerHandler(addResourceHandlers[*provisioningv1.EntraUser]("Entra user", c.entraUserInformer.Informer(), c.enqueueDomain))
	}

	return c
}

func (c *ProvisioningController) registerHandler(registration cache.ResourceEventHandlerRegistration, err error) {
	utilruntime.Must(err)
	c.handlerRegistrations = append(c.handlerRegistrations, registration)
}
...
func (c *ProvisioningController) Run(workers int, stopCh <-chan struct{}) error {
	...
	if c.skipInitialReconcile {
		// The caches being synced only means the initial list reached the informers' stores; events are
		// delivered to the handlers asynchronously, so the "added" events for pre-existing resources can
		// still be in flight. Each handler registration reports when its own pre-sync events have been
		// delivered, so waiting on those guarantees the queue is fully populated before it's drained.
		klog.Info("Waiting for event handlers to be notified of the initial list")
		if !cache.WaitForCacheSync(stopCh, c.handlerSyncFuncs()...) {
			return fmt.Errorf("failed waiting for event handlers to be notified of the initial list")
		}
		c.drainWorkqueue()
	}
...
func (c *ProvisioningController) handlerSyncFuncs() []cache.InformerSynced {
	syncFuncs := make([]cache.InformerSynced, 0, len(c.handlerRegistrations))
	for _, registration := range c.handlerRegistrations {
		syncFuncs = append(syncFuncs, registration.HasSynced)
	}
	return syncFuncs
}

// drainWorkqueue discards the work items that were enqueued by the informers' initial list (i.e. the
// synthetic "added" events fired for every pre-existing resource), so a controller restart doesn't
// re-run pulumi up/refresh against every stack. It must only be called once every handler
// registration reports it has been notified of the initial list, and before workers start.
func (c *ProvisioningController) drainWorkqueue() {
	n := c.workqueue.Len()
	for i := 0; i < n; i++ {
	...
	klog.InfoS("Skipped initial reconcile of pre-existing resources", "count", n)
}
...
func addPlatformHandlers(informer platformInformersv1.PlatformInformer) (cache.ResourceEventHandlerRegistration, error) {
	return informer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
...
func addTenantCategoryHandlers(informer platformInformersv1.TenantCategoryInformer, handler func(platform, name string)) (cache.ResourceEventHandlerRegistration, error) {
	return informer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
...
func addTenantHandlers(informer platformInformersv1.TenantInformer, handler func(*platformv1.Tenant)) (cache.ResourceEventHandlerRegistration, error) {
	return informer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
...
func addResourceHandlers[R ProvisioningResource](resType string, informer cache.SharedIndexInformer, handler func(platform, domain string, target provisioningv1.ProvisioningTargetCategory)) (cache.ResourceEventHandlerRegistration, error) {
