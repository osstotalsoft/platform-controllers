...
## Post-implementation corrections (2026-08-07 whole-branch review fix wave)

The implementation described above shipped with two behaviors that differ from what was designed, discovered when the whole branch diff (all 11 tasks) was reviewed together instead of task-by-task. Both were fixed in the same fix-wave commit series; this section records what actually shipped vs. what was originally designed, for anyone reading this doc as a historical reference.

1. **AzureManagedDatabase mssql connection hostname.** §"Mechanism" implied (and the first implementation assumed) that the sqlmi T-SQL endpoint could be addressed the same way as azuresql's logical server, and the shipped code briefly built it as `fmt.Sprintf("%s.%s.database.windows.net", managedInstance.Name, managedInstance.ResourceGroup)`. That is not a real SQL Managed Instance FQDN — a real one is `<mi-name>.<dnsZone>.database.windows.net`, where `dnsZone` is an Azure-generated virtual-cluster identifier unrelated to the resource group. The fix resolves the real FQDN via `azureSql.LookupManagedInstance(...).FullyQualifiedDomainName` (mirroring how `azureSql.LookupServer` already resolves the azuresql case), in `internal/controllers/provisioning/provisioners/pulumi/azure_managed_db.go`. Port is left at the `mssql` provider's default (1433), not the MI public endpoint's 3342, since this repo has no public-vs-private MI endpoint toggle and every other Azure SQL connection in this codebase assumes private/VNet connectivity.

2. **`deployContainedUser`'s role-membership idempotency.** §"Error handling" states SQL-auth `Script` resources are idempotent "so re-applying an unchanged `User` spec is a no-op" — true for the *unchanged* case, but the first implementation's `readScript` aggregated *all* actual role memberships against a desired state built only from `userSpec.Roles`. That meant removing a role from the spec (or a role being granted out-of-band) made the actual role set a permanent superset of desired, which would never converge — `updateScript` only ever adds role memberships, never drops one, so the mismatch persists on every subsequent apply. The fix scopes `readScript`'s role-membership aggregation to only the roles currently in `userSpec.Roles` (a `r.name IN (...)` filter inside the join), so an unmanaged/removed role can never cause read/desired drift. This is a deliberate, documented limitation, not a bug fix that restores full parity with the original design: **role removal from the spec is not enforced against the database** — the role membership itself is left untouched, only its tracking (and thus its ability to cause drift) stops. See the comments above `deployContainedUser`'s `roleFilter` construction in `internal/controllers/provisioning/provisioners/pulumi/mssql_user.go` for the full rationale, and `TestDeployContainedUserRoleRemovalConverges` in `mssql_user_test.go` for the test proving this behavior.
