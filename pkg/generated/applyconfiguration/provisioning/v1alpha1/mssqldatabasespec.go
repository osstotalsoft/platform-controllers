...
// WithOwnerLoginName sets the OwnerLoginName field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
...
func (b *MsSqlDatabaseSpecApplyConfiguration) WithUsers(values ...*DatabaseUserSpecApplyConfiguration) *MsSqlDatabaseSpecApplyConfiguration {
	for i := range values {
		if values[i] == nil {
...
func (b *MsSqlDatabaseSpecApplyConfiguration) WithCategoryOverrides(entries map[string]*v1.JSON) *MsSqlDatabaseSpecApplyConfiguration {
